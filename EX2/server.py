...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # Here we are using the function's on our sever_socket object of class socket Bind(tuple()) and listen(int)
        # - Bind(tuple) recives a tuple containing (ip address , port) this assigns an ip address and a port number to our socket object
        # - listen(int) receives an integer stating the maximum amount of clients we want our server to handle simuoltanuosly
        # When our server is handling its max amount of clients other clients trying to connect to the server will be put on hold
        # This is odne by threading and creates a new thread for each client - server relationship.
        #
        # * Fill in start (1)
        ...
        while True:
            try:
                ...
                # here we are using the function of the object server_socket: accpet()
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            # Here we are saving to the variable data = the recived message of size 4096 (better if size is a power of 2)
            ...
            try:
                ...
                # * Fill in start (4)
                #Here we are sending the packed response
